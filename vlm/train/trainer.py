...
from data.train_dataloader import ProcessedBatch
...
class Trainer:
    ...
    def _get_input_embeddings(self,
                              batch: ProcessedBatch) -> torch.Tensor:
        """
        Extracts input embeddings from the batch.

        Args:
            batch (ProcessedBatch): Input batch containing tokenized data

        Returns:
            torch.Tensor: Tensor of input embeddings.
        """

        # Check if using distributed training
        if isinstance(self.model, nn.parallel.DistributedDataParallel):
            return self.model.module.get_input_embeddings(**batch)

        # Standard model access
        return self.model.get_input_embeddings(**batch)
    ...
    def _generate_caption(self,
                          ...
                          attention_mask: torch.LongTensor) -> str:
        """
        Generates text description from input embeddings.
        ...
        Returns:
            Generated event sequence description.
        """

        # Setup text generation parameters
        generate_args = {'inputs_embeds': inputs_embeds.unsqueeze(0),
        ...
        # Handle distributed vs regular model
        if isinstance(self.model, nn.parallel.DistributedDataParallel):
            output = self.model.module.generate(**generate_args)
        else:
            output = self.model.generate(**generate_args)

        # Decode generated tokens to text
        return self.tokenizer.decode(output[0].cpu().tolist(),
    ...
    def _log_epoch_statistics(self,
        ...
        Args:
            inputs_embeds (torch.FloatTensor): Embedded representation of input tokens
            batch (ProcessedBatch): Last batch of the epoch to log
            loss (float): Loss for the epoch
            grad_norm (float): Gradient norm for the epoch (only used when AMP is enabled)
        ...
        # Log loss metric and the learning rate if training
        if phase == 'train':
            if self.amp_enabled:
                self.logger.add_scalars_dict(epoch_loss=loss,
                                             epoch_grad_norm=grad_norm,
                                             lr_event_vision_model=self._get_lr(name='backbone'),
                                             lr_aligner=self._get_lr(name='head'))
            else:
                self.logger.add_scalars_dict(epoch_loss=loss,
                                             lr_event_vision_model=self._get_lr(name='backbone'),
                                             lr_aligner=self._get_lr(name='head'))
        ...
        for i in range(min(4, len(batch))):
            # Store ground truth captions
            ground_truth_captions.append(batch.captions[i])

            # Generate model predictions
            generated_captions.append(self._generate_caption(inputs_embeds=inputs_embeds[i],
                                                             attention_mask=batch.attention_mask[i]))

        self.logger.add_table(ground_truth_captions=ground_truth_captions,
    ...
    def run_epoch(self,
        ...
        with torch.set_grad_enabled(is_train):  # Control gradient computation
            for batch_idx, batch in enumerate(pbar):
                ...
                # Forward pass
                inputs_embeds = self._get_input_embeddings(batch)
                with torch.cuda.amp.autocast(enabled=self.amp_enabled):
                    logits = self.model(inputs_embeds=inputs_embeds,
                                        attention_mask=batch.attention_mask).logits
                    loss = self.objective(input=logits.view(-1, logits.shape[-1]),
            ...
            self._log_epoch_statistics(inputs_embeds=inputs_embeds,
                                       batch=batch,
                                       loss=epoch_avg_loss,
