...
from models.event_vlm import build_model
from data.augmenter import build_augmenter
from data.processor import build_processor
...
from data.train_dataloader import build_dataloader
...
def main(rank: int,
    ...
    parser = build_config_parser(config_file_path='configs/train_config.yaml')
    ...
    # Prepare datasets
    train_dataset, val_dataset = build_dataset(data_path=config['data']['data_path'],
                                               train_transform=event_augmenter)

    # Build the event language processor
    ...
    # Build dataloaders
    train_dataloader = build_dataloader(event_text_dataset=train_dataset,
                                        processor=processor,
    ...
    val_dataloader = build_dataloader(event_text_dataset=val_dataset,
                                      processor=processor,
                                      batch_size=config['data']['batch_size'] // world_size,
                                      num_workers=config['data']['num_workers'],
                                      local_rank=local_rank,
    ...
    param_groups = get_parameter_groups(model=model,
    ...
    model = load_pretrained_checkpoint(checkpoint_path=config['event_vision_model']['checkpoint'],
                                       model=model,
                                       input_backbone_name='event_encoder',
                                       output_backbone_name='event_vision_model',
                                       device=device,
                                       verbose=True)

    # Define an objective function
    objective = build_objective(config=config)
    if objective.name == 'classification':  # DeepSeek-VL uses EOS token as PAD token
        objective.set_ignore_index(ignore_index=processor.language_tokenizer.eos_token_id)
    ...
    trainer = Trainer(model=model,
                      tokenizer=processor.language_tokenizer,
                      optimizer=optimizer,
                      objective=objective,
                      scheduler=scheduler,
                      logger=logger,
